...
public class AgentController {
    ...
    @RequestMapping(value = "/agents/{id}/properties", method = RequestMethod.GET)
    public List<Property> getAgentsPropertyById(@PathVariable int id) {
//        return this.as.getAgentById(id);

        List<Property> fromDB = ps.getAllProperties();
        List<Property> result = new ArrayList<Property>();

        for (Property p: fromDB)
        {
            if (p.getAgent().getAid() == id)
                result.add(p);
        }
