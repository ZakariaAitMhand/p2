...
export class PropertyService {
  ...
  constructor(private http:HttpClient) {   }
   ...
   async createProperty(property:Property):Promise<Property>{
    property = await this.http.post<Property>(`http://localhost:8080/agents/${property.agent.aid}/properties`,property).toPromise();
  ...
  async getAllPropertyTypes():Promise<Array<PropertyType>>{
    const propertyTypes:Array<PropertyType> = await this.http.get<Array<PropertyType>>("http://localhost:8080/propertytypes").toPromise();
    return propertyTypes;
  }

  async createPropertyType(propertyType:PropertyType):Promise<PropertyType>{
    propertyType = await this.http.get<PropertyType>(`http://localhost:8080/propertytypes`).toPromise();
    return propertyType;
  }

  async getPropertyTypeById(id:number):Promise<PropertyType>{
    const propertyType:PropertyType = await this.http.get<PropertyType>(`http://localhost:8080/agents/${id}`).toPromise();
    return propertyType;
  }
}
